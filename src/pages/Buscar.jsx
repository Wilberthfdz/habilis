// Buscar.jsx — Página de búsqueda (conecta con Firebase)
...
export default function Buscar({ nav, params }) {
  ...
  useEffect(() => {
    buscarTecnicos({ oficio: params?.oficio }).then(t => { setTecnicos(t); setLoading(false); });
  }, []);
  ...
  const s = {
    ...
    btn:    { background:"#D97706", color:"#fff", border:"none", borderRadius:"10px", padding:"10px 18px", fontSize:"13px", fontWeight:700 },
  };
    ...
    <div style={s.page}>
      ...
      <div style={{ background:"#fff", borderBottom:"1px solid #E5E7EB", padding:"14px 20px" }}>
        <div style={{ display:"flex", gap:"8px", maxWidth:"960px", margin:"0 auto" }}>
          <input style={s.inp} value={q} onChange={e => setQ(e.target.value)} placeholder="¿Qué necesitas? (electricista, plomero...)" />
          <button style={s.btn} onClick={() => buscarTecnicos({ oficio: q }).then(setTecnicos)}>Buscar</button>
        </div>
      </div>
      <div style={{ maxWidth:"960px", margin:"0 auto", padding:"20px" }}>
        {loading
          ? <p style={{ color:"#6B7280", textAlign:"center", padding:"40px" }}>Cargando técnicos...</p>
          : tecnicos.length === 0
                ...
                {tecnicos.map(t => (
                  <div key={t.id} style={s.card} onClick={() => nav("perfil", { tecnicoId: t.id })}>
                    <div style={{ display:"flex", gap:"14px", alignItems:"flex-start" }}>
                      ...
                      <button style={{ ...s.btn, padding:"7px 14px", fontSize:"12px" }}>Contactar</button>
                    </div>
                  </div>
                ))}
              </div>
